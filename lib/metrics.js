...
Metrics.prototype.start = function(key) {
  ...
  // Delete after given timeout, don't report the time.
  var timeout = setTimeout(function() {
    if (metrics.remove(key)) {
      log.warn('metrics timeout for ' + key);
    }
  }, TIMEOUT);

  metrics.timeouts[key] = timeout;
};
...
Metrics.prototype.end = function(key, startKey) {
  ...
  if (start) {
    ...
    this.remove(startKey);
  }

  return diff;
};

// Remove the start time for given key. Return success status.
Metrics.prototype.remove = function(key) {
  var active = Boolean(this.times[key]);
