...
var Http2Proxy = function(options) {
  ...
  function handleContent(request, response) {
    ...
    if (request.url === '/bugreport') {
      return handleClientReport(request, response);
    }
  ...
  */
  ...
  this.on('connection', function(socket, endpoint) {
    numConnections++;
    log.debug('new connection, %d total', numConnections);

    endpoint.on('close', function() {
      numConnections--;
      log.debug('lost connection, %d total', numConnections);
    });
  });
};
